...
2. 전화 번호  
ex) '02-111-2222' > /\d{2,3}-\d{3,4}-\d{4}/

3. 휴대폰 번호  
ex) '010-1234-5678' > /^\d{3}-\d{3,4}-\d{4}$/

4. 웹사이트 주소  
ex) 'https://zeuskwon.com' > /(http(s)?:\/\/)([a-z0-9\w]+\.*)+[a-z0-9]{2,4}/

5. 이메일 주소  
ex) 'zeuskwon@epozen.com' > /^[a-zA-Z0-9+-\_.]+@[a-zA-Z0-9-]+\.[a-zA-Z0-9-.]+$/
