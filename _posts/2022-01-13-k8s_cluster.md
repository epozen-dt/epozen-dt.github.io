...
그럼 이제 Docker와 k8s를 설치해 볼겁니다. 그 전에 우선 해줘야 하는 귀찮을 일들이 있네요. 하지만 반드시 해줘야 하는 것들이니 잘 따라서 해 주시면 됩니다.

SELinux 모드를 Permissive 모드로 변경, 방화벽/NetworkManager/Swap 비활성화, iptables 커널옵션 활성화, Host 정보 등록입니다.
...
마지막 단계는 CNI 플러그인 설치입니다. 다양한 플러그인이 있는데 여기서에서는 가장 많이 사용하는 calico라는 플러그인을 설치해 볼 겁니다.

calico.yaml 파일을 다운로드 하고, 파일의 "192.168.0.0\\\/16" 을"20.96.0.0\\\/12"로 수정합니다.
